...
% Define the state equation. Eventually we will want to choose between a few 
% different equations for f.  (The max conditions to avoid negative values probably aren't necessary).  

f = @(x, h) max( max((x - h),0) * (1 - max((x - h),0) ./ 100) + max((x - h),0), 0);

% Alternative state equations.  
%f = @(x, h) max( max((x - h),0) * exp(1.1 * (1 - max((x - h),0) ./ K)), 0) ;
%f = @(x, h) max( 2 * max((x - h),0) / (1 + 4 * max((x - h),0)), 0); % K = 0.25


% Define the state space.  Here we 
x_grid = [0:5:150];
